...
pub static assets: Lazy<Assets> = Lazy::new(|| Assets {
    main_css: CssAsset {
        url: "main.css",
        relative_path: "target/tailwind/built.css",
    },
    hasui_light_jpeg: ImageAsset::new(
        "hasui-light.jpeg",
        "Alt",
        include_bytes!("../routes/hasui_light_jpeg/hasui_light.jpeg"),
    ...
    hasui_dark_jpeg: ImageAsset::new(
        "hasui-dark.jpeg",
        "Alt",
        include_bytes!("../routes/hasui_dark_jpeg/hasui_dark.jpeg"),
...
pub struct ImageAsset {
    ...
    pub bytes: &'static [u8],
    pub lqip: &'static str,
    pub resized_variants: HashMap<ImageWidthU32, Vec<u8>>,
    srcset: String,
}

type ImageWidthU32 = u32;
...
impl ImageAsset {
    pub fn new(
        ...
        let resized_variants = Self::create_resized_variants(image);
        let srcset = Self::create_srcset(url, width);
    ...
    fn create_resized_variants(image: DynamicImage) -> HashMap<u32, Vec<u8>> {
        let width = image.width();

        Self::available_widths(width)
            .into_iter()
            .map(|target_width| {
                // Don't upscale images.
                if target_width > width {
                    return (target_width, image.clone().into_bytes());
                }

                let resized_bytes = image.resize_to_width(target_width).into_bytes();
                (target_width, resized_bytes)
            })
    ...
    fn create_srcset(url: &str, image_width: u32) -> String {
        Self::available_widths(image_width)
            .into_iter()
            .map(|width| {
                let url_with_width = Self::url_with_width(url, width);
                format!("{url_with_width} {width}w")
            })
            .collect::<Vec<String>>()
            .join(", ")
    }
    ...
    fn url_with_width(url: &str, width: u32) -> String {
        format!("{}?width={}", url, width)
    }
}
